# 九州通接口边界

## 原则

九州通接口属于适配器，不得直接写 `stock` 或 `gsp_batch_stock`。内部业务事务先提交，随后在同一事务中写入
`gsp_integration_outbox`；后台发送器按幂等键投递、重试和对账。外部回执也先保存原始报文，再由应用服务验证状态转换。

当前仓库只实现适配器中立的出站箱治理：基于规范化载荷的确定性 SHA-256 幂等键、并发批次认领、
指数退避、死信状态，以及带电子签名和审计追踪的人工重放。九州通网络客户端、鉴权签名、字段映射、
回执入站和正式对账均刻意留待取得接口规范及测试环境后实现。

## 正式开发前需取得的资料

- 测试/生产地址、网络白名单、TLS 与证书要求。
- 鉴权、签名、时间戳、防重放、密钥轮换方式。
- 药品、客户、仓库、批号、单位和状态代码表。
- 订单、发货、入库回执、库存、退货、追溯码等报文规范与示例。
- 同步/异步模式、超时、限流、重试窗口、错误码和补偿规则。
- 幂等字段、业务唯一键、报文版本和变更通知机制。
- 日终对账文件、差异处理、历史重发和人工干预流程。

## 推荐消息类型

| 消息 | 方向 | 内部触发点 |
|---|---|---|
| `PRODUCT_MASTER_CHANGED` | 本系统→九州通 | 药品质量主数据批准后 |
| `PARTNER_QUALIFIED` | 本系统→九州通 | 合作方质量批准后 |
| `PURCHASE_ORDER_RECEIVED` | 九州通→本系统 | 外部采购/配送单通过校验后 |
| `BATCH_ACCEPTED` | 本系统→九州通 | 收货验收放行后 |
| `SHIPMENT_CONFIRMED` | 本系统→九州通 | 独立出库复核通过并实际发运扣减库存后 |
| `INVENTORY_SNAPSHOT` | 本系统→九州通 | 日终或按约定周期 |
| `STOCKTAKE_COMPLETED` | 本系统→九州通 | 批号库存盘点复核完成或批准差异调整执行后 |
| `SALES_RETURN_RECEIVED` | 本系统→九州通 | 销后退货关联原发运并完成隔离收货后 |
| `SALES_RETURN_INSPECTED` | 本系统→九州通 | 退货质量检验完成并判定回库/拒收后 |
| `NONCONFORMING_DISPOSITION_APPROVED` | 本系统→九州通 | 不合格品处置经独立质量批准后 |
| `NONCONFORMING_DESTROYED` | 本系统→九州通 | 不合格品完成监督销毁并登记证明后 |
| `PURCHASE_RETURN_APPROVED` | 本系统→九州通 | 购进退出通过独立质量批准后 |
| `PURCHASE_RETURN_DISPATCHED` | 本系统→九州通 | 购进退出实际发运并完成库存扣减后 |
| `RECALL_ACTIVATED` | 本系统→九州通 | 召回独立审批并锁定相关批次后 |
| `RECALL_TARGET_UPDATED` | 本系统→九州通 | 购货方通知或回收进度更新后 |
| `RECALL_PROGRESS_REPORTED` | 本系统→九州通 | 按召回等级登记阶段进展报告后 |
| `RECALL_CLOSED` | 本系统→九州通 | 召回数量核对与独立关闭复核后 |
| `RECALL_COMPLETION_REPORTED` | 本系统→九州通 | 召回完成报告登记监管报送引用后 |
| `MAINTENANCE_PLAN_COMPLETED` | 本系统→九州通 | 养护检查完成并经独立质量复核后 |

召回演练属于企业内部质量活动，不默认进入九州通出站箱，也不会复用真实召回通知事件。只有双方正式接口规范明确要求后，才增加演练类报文。

## 必测场景

重复报文、乱序回执、超时后成功、部分明细失败、批号不一致、单位换算、过期资质、质量锁定、库存不足、
追溯码重复、签名错误、接口版本变化、断网恢复、历史重放和日终对账差异。
