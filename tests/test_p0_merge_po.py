...
def _seed(db):
    ...
    for name in ("procurement", "quality", "receiver", "inspector"):
        user = User(
            username=f"po{name}-{suffix}", hashed_password="x", full_name=name, role=UserRole.OPERATOR
        )
        db.add(user)
        users.append(user)
    ...
    profile = GspDrugProfile(
        ...
        registration_document_ref="test://reg",
        nmpa_verification_ref="test://nmpa",
        status="APPROVED",
        approved_by=users[1].id,
        created_by=users[1].id,
    )
    ...
    db.add(
        GspSupplierProductAuthorization(
            ...
            goods_id=goods.id,
            authorization_ref="test://auth",
            authorization_sha256="a" * 64,
            authorization_size_bytes=1,
            scope_description="获准供应",
    ...
    add_verified_partner_evidence(
        db, partner=supplier, verifier_id=users[1].id, valid_to=date.today() + timedelta(days=365)
    )
    db.flush()
...
def test_po_draft_cancel_and_quality_reject():
    ...
    try:
        ...
        c = cancel_purchase_order(
            db, order_id=o.id, actor_id=procurement.id, reason="录入有误作废重开", source_ip="127.0.0.1"
        )
        assert c.status == "CANCELLED" and c.cancelled_by == procurement.id and c.cancellation_reason
        with pytest.raises(WorkflowError):
            submit_purchase_order(
                db, order_id=o.id, actor_id=procurement.id, reason="x", source_ip="127.0.0.1"
            )

        o2 = _order(db, users, wh, goods, supplier, f"PO-R-{uuid4().hex[:6]}")
        submit_purchase_order(
            db, order_id=o2.id, actor_id=procurement.id, reason="提交审批", source_ip="127.0.0.1"
        )
        with pytest.raises(WorkflowError, match="不能驳回"):
            reject_purchase_order(
                db, order_id=o2.id, actor_id=procurement.id, reason="自驳", source_ip="127.0.0.1"
            )
        r = reject_purchase_order(
            db, order_id=o2.id, actor_id=quality.id, reason="供货范围不符，驳回重办", source_ip="127.0.0.1"
        )
...
def test_expired_batch_blocked_at_receipt_and_release():
    ...
    try:
        ...
        submit_purchase_order(
            db, order_id=o.id, actor_id=procurement.id, reason="提交质量审批", source_ip="127.0.0.1"
        )
        approve_purchase_order(
            db, order_id=o.id, actor_id=quality.id, reason="质量批准通过", source_ip="127.0.0.1"
        )
        db.flush()
        ...
        with pytest.raises(WorkflowError, match="已过有效期"):
            create_receipt(
                ...
                payload=ReceiptCreate(
                    receipt_no=f"RCV-E-{uuid4().hex[:6]}",
                    purchase_order_id=o.id,
                    delivery_document_no="DEL-E",
                    arrived_at=datetime.now(),
                    items=[
                        ReceiptLineCreate(
                            purchase_order_item_id=item.id,
                            batch_no=f"B-{uuid4().hex[:4]}",
                            production_date=date.today() - timedelta(days=400),
                            expiry_date=date.today() - timedelta(days=1),
                            quantity=Decimal("5.000"),
                            location_id=loc.id,
                            inspection_report_no="R",
                            traceability_code="T",
                        )
                    ],
                    reason="过期收货测试",
        ...
        rc = create_receipt(
            db,
            payload=ReceiptCreate(
                receipt_no=f"RCV-O-{uuid4().hex[:6]}",
                purchase_order_id=o.id,
                delivery_document_no="DEL-O",
                arrived_at=datetime.now(),
                items=[
                    ReceiptLineCreate(
                        purchase_order_item_id=item.id,
                        batch_no=f"B2-{uuid4().hex[:4]}",
                        production_date=date.today() - timedelta(days=30),
                        expiry_date=date.today() + timedelta(days=700),
                        quantity=Decimal("5.000"),
                        location_id=loc.id,
                        inspection_report_no="R2",
                        traceability_code="T2",
                    )
                ],
                reason="正常收货",
            ),
        ...
        record_receipt_sampling(
            ...
            payload=ReceiptSampling(
                sampling_plan_ref="SOP-001",
                sampling_method="随机",
                sample_quantity=Decimal("1"),
                sampling_record_no=f"SP-{uuid4().hex[:6]}",
                reason="完成抽样记录",
            ),
