...
当前版本：`0.18.0`。当前定位是**可继续开发和验证的 GSP 工程基线**，尚不是可直接用于
药品经营活动的商业成品。
...
| 第十五阶段 | 合作方年度评审/质量体系调查、风险与 CAPA、自查内审、偏差/投诉/事故/不良反应、培训与岗位考核、受控文件、设备验证校准和专项经营范围 | 已完成（软件控制） |
...
| 第十九阶段 | 真实受控附件存储：服务端上传与哈希、不可变内容寻址存储、gspf 令牌、ATTACHMENT_POLICY、四域业务记录绑定（首营/授权/注册/承运）与上传人停用保护 | 软件能力已完成；生产启用 ATTACHMENT_POLICY=enforce 后形成强制闭环（生产配置校验拒绝 warn） |
...
| 暂缓项 | CSV 验证文件包（VP/URS/RA/FS/DS/RTM/IQ/OQ/PQ/报告/放行） | 已建立模板和协议，正式执行仍需在目标环境完成 |
| 部署验收 | 目标环境秘密管理、备份定时器、异地介质/告警和首次恢复演练 | 待完成 |

当前代码范围内业务与技术控制已收口。明确暂缓四项外部/交付阶段工作：药品追溯平台、
实际温湿度网关及告警渠道、九州通正式适配与联调、完整 CSV 验证文件包。
...
- 供应商与获准供货品种建立独立有效期关联；关联的维护与批准分离，缺失、未批准、暂停或过期时阻断采购建单、审批、收货和验收。
- 供应商品种目录支持最多 1000 行的原子批量导入，按货物条码和批准文号双重核对；导入结果统一待独立审批，并在合规概览展示待审、临期和过期数量。
- 药品质量主数据保存注册批准档案与 NMPA/企业核验引用；任何变更都撤销原批准并要求独立重新审批。
- 批号、生产日期、有效期、追溯码和冷链温度信息校验。
- 停用用户代替物理删除，保留历史记录中的操作人引用。
- 合作方年度评审与质量体系调查支持独立批准；有条件通过必须保存整改计划、期限和独立关闭证据。
- 质量风险、偏差、自查、内审、投诉、质量事故、不良反应和 CAPA 采用可审计状态机与职责分离。
- 培训、岗位资格与职责考核保留成绩、结果、证据和独立复核；失败结果不会显示为合格。
- 受控文件覆盖修订、批准、生效、旧版副本召回、回收和销毁；设施设备覆盖验证、校准、维护及失败停用。
...
- 客户端先向 `/api/gsp/electronic-signatures/challenges` 提交拟执行动作和完整请求正文，再用相同正文及 `X-GSP-Signature-Token` 调用受监管接口。
...
- 召回关闭后按批准的工作日历生成10个工作日完成报告期限；日期例外需经电子签名批准并保留审计证据。
- 近效期预警、重点养护选取和停止销售阈值可由质量人员电子签名批准；每日扫描自动生成证据，达到停销阈值时创建质量锁定并将同批库存置为 `HOLD`。

### 批号库存盘点

- 盘点范围以批号库存记录为最小单位，支持手工、全量、近效期和系统抽样选取；质量批准时固化账面数量、预留数量与并发版本。
- 实盘阶段对盘点人员隐藏账面数量和差异，差异必须记录原因；盘点批准人与实盘人员分离。
- 质量复核可要求差异复盘或批准调整；实盘、差异批准和调整执行三项职责必须由不同人员承担。
- 批准后冻结范围内数量及预留交易；调整执行前再次核对库存数量和 `lock_version`。
- 差异批准强制引用偏差/CAPA，盘点表受控打印保存模板版本、唯一份号、固化快照和 SHA-256。
...
- 审计链校验结果保存事件数量、结果、断点和证据引用；生产计划任务从有效审计岗位选择责任人。
- 九州通等外部系统通过事务出站箱对接；内部已具备确定性幂等键、并发认领、退避重试、死信和电子签名重放，正式协议适配器仍未实现。
- 提供批号追溯、审计链校验和合规概览接口。
...
- **真实受控附件**：`POST /api/gsp/files` 服务端计算 SHA-256、不可变内容寻址存储
  ...
  服务端 sha/大小为准；`ATTACHMENT_POLICY=enforce`（生产必须，`Settings.validate()` 拒绝
  生产以 warn 启动；warn 仅用于历史数据迁移/过渡）；下载前完整性校验、篡改 410；
...
| 供应商品种目录 | `/api/gsp/partners/{partner_id}/products` | 供应商与获准供货品种关联、独立批准、暂停和有效目录查询 |
| 供应商品种批量导入与预警 | `/api/gsp/partners/{partner_id}/products/bulk-import`、`/api/gsp/supplier-product-authorizations` | 原子批量回填、待审批及临期/过期记录查询 |
...
| 我的 CAPA | `GET /api/gsp/quality-system/capas/me` | 责任人仅查询并提交本人 CAPA 完成证据 |
| 老 GSP 历史归档 | `/api/gsp/legacy-archive` | 批次验证、摘要校验导入、独立数量核对、只读检索和 JSONL 导出 |
